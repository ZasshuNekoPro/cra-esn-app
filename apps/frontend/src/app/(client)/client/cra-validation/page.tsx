...
import { ClientCraActions } from './ClientCraActions';
...
export default async function ClientCraValidationPage(): Promise<JSX.Element> {
  const { items } = await craApi.getPendingClient().catch(() => ({ items: [], count: 0 }));

  return (
    <div className="max-w-4xl mx-auto">
      <h1 className="text-2xl font-bold text-gray-900 mb-2">Validation des CRA</h1>
      <p className="text-sm text-gray-500 mb-8">
        CRA en attente de votre signature ({items.length})
      </p>
                ...
                return (
                  <tr key={item.craMonthId} className="text-gray-700">
                    ...
                    <td className="px-6 py-4">
                      <ClientCraActions craMonthId={item.craMonthId} />
                    </td>
                  </tr>
                );
              })}
