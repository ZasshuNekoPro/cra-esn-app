...
export default async function ClientProjectsPage(): Promise<JSX.Element> {
  const projects = await projectsApi.listForClient().catch(() => []);

  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Projets</h1>
        <p className="text-sm text-gray-500 mt-1">Projets liés à vos missions.</p>
      </div>

      {projects.length === 0 ? (
        <div className="text-center py-16 text-gray-500">
          <p className="text-lg font-medium mb-1">Aucun projet pour le moment</p>
          <p className="text-sm">Les projets associés à vos missions apparaîtront ici.</p>
        </div>
      ) : (
